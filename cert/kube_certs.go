...
func NewSealosCertMetaData(certPATH, certEtcdPATH string, apiServerIPAndDomains []string, SvcCIDR string, nodeNames []string, nodeIPs []string, DNSDomain string) (*SealosCertMetaData, error) {
...
func (meta *SealosCertMetaData) apiServerAltName(certList *[]Config) {
	...
	for _, nodeName := range meta.NodeNames {
...
func (meta *SealosCertMetaData) etcdAltAndCommonName(certList *[]Config) {
	...
	for _, nodeName := range meta.NodeNames {
		altname.DNSNames[nodeName] = nodeName
	}

	for _, nodeIP := range meta.NodeIPs {
		altname.IPs[net.ParseIP(nodeIP).To4().String()] = net.ParseIP(nodeIP).To4()
	}

	(*certList)[EtcdServerCert].CommonName = "etcd"
	(*certList)[EtcdServerCert].AltNames = altname
	(*certList)[EtcdPeerCert].CommonName = "etcd"
